...
- 完成動画の文字起こし（実尺タイムコード付き）が渡された場合は、それを根拠にYouTubeチャプター用の目次(chapters)も作る。
  ...
  ラベルはSEOを意識する：視聴者が検索しそうな具体語（病名・治療法・職業・出来事・固有名詞）を入れた8〜18字。
  「移動」「帰宅」「診察」のような中身のない一般語だけのラベルは禁止（例:×「診察」→○「FIP疑いの猫を診察」）。
...
const DELIVER_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      description: { type: "string", description: "概要欄テキスト（文章体250〜400字、箇条書き禁止）" },
