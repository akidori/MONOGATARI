...
# ルール
- 場所の見出しは {kind:"location", label, time?, day?}。その配下に scene を順に並べる。素材に「1日目/2日目」「DAY1/DAY2」等の撮影日区切りがあれば、その日以降の location に day（1,2,…）を付ける
- location の label は必ず自然な日本語（漢字・かな）で書く。素材がローマ字表記（例: nagoyaeki）でも、意味の分かる地名なら日本語に直す（nagoyaeki→名古屋駅）。読み取れない固有名詞のみ元の表記を残す
      ...
      rows: {
        ...
        items: {
          ...
          properties: {
            ...
            day: { type: "number", description: "撮影日（1=1日目、2=2日目…）。複数日撮影のとき location に付ける。省略時は1日目" },
...
# ルール
- location の label は必ず自然な日本語（漢字・かな）で書く。メッセージがローマ字表記（例: nagoyaeki）でも、意味の分かる地名なら日本語に直す（nagoyaeki→名古屋駅）。読み取れない固有名詞のみ元の表記を残す
...
const REVIEW_SYSTEM = `あなたは一日密着ドキュメンタリー構成台本の校正者です。渡された「構成台本(JSON)」を読み、下記の4観点だけを厳密にチェックして report_review ツールで指摘リストを返してください。

# チェック観点（この4つだけ）
0. 内容の重複（category="重複"）【最重要】
   - 別のシーンで同じ話・同じエピソード・同じ説明・同じ主張が繰り返されていないかを台本全体を通して確認する
   - 例：シーンAで「サービスの成り立ち」を語り、シーンCでもほぼ同じ成り立ちの話をしている／同じ数字・同じ例え・同じ決め台詞が複数シーンに出てくる
   - 同じ話題に触れていても「角度が違う」（事実紹介と、想い・裏話など）なら重複としない。実質的に同じ内容が繰り返される場合だけ挙げる
   - detail には重複している両方のシーン見出しを挙げ、どこが同じ内容かを引用で示す。suggestion には「どちらに寄せてどちらを削る／別の角度に変える」の提案を書く
   - rowId には削る・変える候補側（通常は後に出てくる方）のシーンidを入れる
      ...
      issues: {
        ...
        items: {
          ...
          properties: {
            ...
            category: { type: "string", enum: ["重複", "誤字脱字", "質問と回答の逆転", "未記入", "その他"] },
...
- 「重複」の指摘は自動で直さない（どちらのシーンを残すかは人の編集判断）→skipped
- note に何をしたか1〜2行`;
...
# Obsidian承認済みナレッジ（表現・コンプライアンス規定 / Production/Rule、2026-08-26同期）
- 施術・治療効果を語る箇所には「※個人の感想です／効果には個人差があります」の注意書きがテロップに無ければ懸念として出す（category: expression）
- 角が立つ発言・他者批判が本人の発言として書かれている場合は懸念として出す（第三者に語らせる構成が原則。category: expression）
- 競合他社・取引先の具体的な社名がセリフ・テロップ・概要欄に残っている場合は懸念として出す（一般化表現への置換を提案。category: sensitive）
- 出演者の社内用語・肩書きの言い間違いを編集側で無断修正したまま確認依頼が無い場合は懸念として出す（category: approval）
- 差別的・侮蔑的な表現、本人を否定する言葉（変人／異端／非常識／気持ち悪い 等）があれば懸念として出す（category: expression）
- 金銭を誇大に断定する表現（誰でも稼げる／必ず儲かる 等）があれば懸念として出す（category: expression）
- 企業案件（クライアントワーク）ではオフブランドな演出、テロップ視認性不足、モニター等の映り込み・イメージ画像への注釈漏れも確認する（category: brand）

# 個別承認（固定）の扱い
meta.deliverApproved は、制作側が納品完了画面で「固定」した項目とその固定日時。これが全社ルールの「本編・サムネ・タイトル・概要欄をそれぞれ個別に承認する」の承認記録にあたる。
- deliverApproved にキーがある項目については「個別承認記録がない」類の懸念を出してはいけない（承認済みとして扱う）
- キーが無い項目についてだけ、承認が必要かどうかを判断する。ただし承認の有無だけを理由に severity:"block" にはせず、内容そのものに問題がある場合に block を使う
...
async function preflightWithClaude(project, env) {
  ...
  const safeProject = {
    ...
    meta: {
      ...
      // 納品完了タブで「固定」した項目＝個別承認の記録（値は固定日時のISO）。2026-08-28
      deliverApproved: meta.deliverApproved || {}
    },
...
- **概要欄に書く情報は「動画内に実際に出てくる文言」だけに限る**（2026-08-26 AKルール）。根拠にできるのは台本の原稿・本人の発言・（あれば）完成動画の文字起こしに出てくる事実と言い回しのみ。そこに出てこない経歴・数字・形容・エピソードは、たとえ一般に知られていそうでも一切書かない。動画を見た人が「そんな話あったっけ？」となる記述をゼロにする
...
# 表現・コンプライアンス規定（Obsidian Production/Rule、2026-08-26同期。クライアントワーク全般に適用）
- 施術・治療効果を断定・保証する表現（必ず治る／絶対効く 等）は使わない
- 差別的・侮蔑的な表現、本人を否定する言葉（変人／異端／非常識／気持ち悪い 等）は使わない。意外性・煽りは「認識の裏切り」で作り、人物を否定する言葉に頼らない
- 競合他社・取引先の具体的な社名は出さない。「他社」「関連企業」等に一般化する
- 金銭を誇大に断定する表現（誰でも稼げる／必ず儲かる 等）は使わない
- 性的・暴力的・政治/宗教に踏み込む表現は使わない
- 角が立つ発言・他者への批判を、本人が自分で言ったかのような文言として新規生成しない
...
D. 原稿(script)の書式
  ...
  - ロケ名(location.label)は必ず自然な日本語（漢字・かな）で書く。素材がローマ字表記（例: nagoyaeki）でも、意味の分かる地名なら日本語に直す（nagoyaeki→名古屋駅）。読み取れない固有名詞のみ元の表記を残す
      ...
      rows: {
        ...
        items: {
          ...
          properties: {
            ...
            label: { type: "string" },
            time: { type: "string" },
            day: { type: "number", description: "撮影日（1=1日目、2=2日目…）。既存locationのdayは変更依頼がない限り維持する" },
            address: { type: "string" },
            note: { type: "string" },
            type: { type: "string", enum: ["インサート", "ブリッジ", "VLOG", "解説系", "訴求"] },
            sec: { type: "number" },
